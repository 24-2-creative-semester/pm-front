...
const ExerciseMain: React.FC = () => {
    ...
    const fetchTodayExercises = async () => {
      try {
        ...
        const response = await fetch(
          "http://172.16.4.171:8080/exercise/todayList",
          {
            method: "GET",
            headers: {
        ...
        if (response.ok && data.isSuccess) {
          // result 데이터를 "오늘의 운동" 구조에 맞게 변환
          const updatedExercises = data.result.map((exercise: any) => ({
            name: exercise.exerciseName,
            calories: exercise.exerciseCalories,
            duration: new Date(exercise.exerciseDate).toLocaleTimeString(), // 시간 포맷 변환
            exerciseId: exercise.exerciseId, // 삭제에 필요
          }));
    ...
    const handleRemoveExercise = async (exerciseId: number) => {
      try {
        const accessToken = await AsyncStorage.getItem("accessToken");
        const response = await fetch(
          `http://172.16.4.171:8080/exercise/deleteExercise?exerciseId=${exerciseId}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `${accessToken}`,
            ...
            {exercises.map((exercise, index) => (
              <View key={index} style={styles.exerciseItem}>
                ...
                <Text style={styles.exerciseInfo}>
                  {exercise.calories}kcal / {exercise.duration}
                </Text>
                <TouchableOpacity
